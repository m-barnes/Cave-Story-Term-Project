...
void AnimatedSprite::addAnimation(int frames, int x, int y, string name, int width, int height, Vector2 offset) {
	...
	//for each frame of the animation grab a rectangle of a specified height and store it in the vector declared above. Each subsequent rectangle will start at the ending 'x' location of the previous rectangle. 
	for (int i = 0; i < frames; i++) {
		SDL_Rect newRect = { (i + x) * width, y, width, height };
		rectangles.push_back(newRect);
	}
	//take rectangles and store it in my animations map. Store it by name and by the rectangles vector. Use 'pair' to make sure they stay together.
